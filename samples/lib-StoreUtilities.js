...
class StoreUtilities {
	...
	async getDataFromCsv(url, columnName, logger = true) {
		const buster = this.buster
		if (logger) this.log(`Getting data from ${url}...`, "loading")
		...
		if (match) {
			...
			if (logger) this.log(`Got ${result.length} lines from csv.`, "done")
			return result
		} else {
