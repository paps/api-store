...
const openProfile = async (page: puppeteer.Page, url: string) => {
	...
	// if (await page.$("blockquote a[href=\"//www.craigslist.org/\"]")) {
	// 	return { error: "Invalid page!", timestamp: (new Date()).toISOString() }
	// }
	try {
...
(async () => {
	...
	if (Array.isArray(pageArray)) {
		...
		utils.log(`Scraped ${result.length} pages in total.`, "done")
