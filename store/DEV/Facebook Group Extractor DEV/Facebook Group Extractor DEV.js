...
const scrapeMainPage = (arg, callback) => {
	...
	for (const result of results) {
        ...
        if (result.querySelector(".friendButton")) newInfos.category = "Friend"
        let dateAndJob = result.querySelectorAll("._60rj")
        for (let data of dateAndJob){
            if (newInfos.firstLine) {
                newInfos.secondLine = data.textContent
            } else { 
                newInfos.firstLine = data.textContent
            }
        }
        if (result.querySelector("._2zy5 a")) newInfos.inCommon = result.querySelector("._2zy5 a").textContent 
        infos.push(newInfos)
    }
...
nick.newTab().then(async (tab) => {
    ...
    // await utils.saveResults(result, craftCsvObject(result), "resultsFB")
    console.log("avant", result)
    const results = filterResult(result, "profileURL")
