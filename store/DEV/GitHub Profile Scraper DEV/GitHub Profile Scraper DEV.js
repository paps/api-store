...
const isGithubUrl = (url) => {
    try {
        return (new url_1.URL(url)).hostname === "github.com";
    }
    catch (err) {
        return false;
    }
...
const scrapeUser = () => {
    ...
    if (commitsCount) {
        ...
        const _count = regex ? regex.filter((el) => !isNaN(parseInt(el, 10))).pop() : "0";
    ...
    profile.organizations = Array.from(organizations).map((el) => el.href);
    ...
    profile.pinnedRepos = Array.from(pinnedRepos).map((el) => el.querySelector("span.d-block a") ? el.querySelector("span.d-block a").href : null);
    ...
    for (const el of infos) {
        if (el.parentNode && el.parentNode.childNodes && el.textContent) {
            ...
            const filtered = vals.filter((content) => content.nodeType === Node.TEXT_NODE);
            const values = filtered.map((ele) => ele && ele.textContent ? ele.textContent.trim() : "");
            const _final = values.filter((_el) => _el);
