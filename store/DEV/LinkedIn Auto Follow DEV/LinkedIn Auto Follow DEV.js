...
const follow = async (selector, tab) => {
    ...
    try {
        ...
        utils.log("Button clicked but could not verify if the user was followed.", "warning")
    }
}
...
const addFollow = async (url, tab, unfollowProfiles, disableScraping) => {
    ...
    const selectors = [".pv-s-profile-actions--follow", // follow button already available (third+ circle)
        ".pv-s-profile-actions--unfollow", // unfollow button already available (third+ circle)
        ".pv-dashboard-section", // Own profile detected
        ".pv-s-profile-actions__overflow-toggle" // two-step follow with click on (...) required
    ]
    let selector
    try {
    ...
    utils.log(`SELECTOR ${selector}`, "warning")
    ...
    if (!checkDb(currentUrl, db)) {
        ...
        if (selector === selectors[3]) { //  Case when you need to use the (...) button before and (un)follow them from there
            await tab.click(".pv-s-profile-actions__overflow-toggle")
            selector = await tab.waitUntilVisible([".pv-s-profile-actions--unfollow", ".pv-s-profile-actions--follow"], 5000, "or")
        }
        // Case when you can (un)follow directly
        if (selector === selectors[0] || selector === selectors[1]) {
            if (selector === ".pv-s-profile-actions--follow" && unfollowProfiles){
                utils.log(`We weren't following ${url}.`, "done")
            } else if (selector === ".pv-s-profile-actions--unfollow" && !unfollowProfiles){
                utils.log(`We were already following ${url}.`, "done")
            } else {
                await follow(selector, tab)
                utils.log(`${unfollowProfiles ? "Unf" : "F"}ollowed ${url}.`, "done")
            }
        } else if (selector === selectors[2]) { // Case when you put your own profile
...
nick.newTab().then(async (tab) => {
    const [sessionCookie, spreadsheetUrl, unfollowProfiles, numberOfAddsPerLaunch, columnName, hunterApiKey, disableScraping] = utils.checkArguments([
        { name: "sessionCookie", type: "string", length: 10 },
        { name: "spreadsheetUrl", type: "string", length: 10 },
        { name: "unfollowProfiles", type: "boolean", default: false },
