...
const openProfile = async (tab, url, noScraping = true) => {
	...
	if (noScraping) {
		...
		const scrapedProfile = await linkedInScraper.scrapeProfile(tab, url.replace(/.+linkedin\.com/, "linkedin.com"))
...
nick.newTab().then(async (tab) => {
	...
	linkedInScraper = new LinkedInScraper(utils, hunterApiKey || null, nick) // TODO: FIX
	...
	let reservedColumns = LinkedInScraper.csvFields()
	let columns = [ columnName, ...getMessageTags(message) ]
	columns = columns.filter(el => disableScraping ? el !== "firstName" : !reservedColumns.includes(el))
