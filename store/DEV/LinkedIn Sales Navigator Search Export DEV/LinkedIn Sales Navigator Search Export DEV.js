...
const createUrl = (search, circles) => {
	...
	return (`https://www.linkedin.com/sales/search?keywords=${encodeURIComponent(search)}`) // TODO: test + encodeURI
}
...
const scrapeResults = (arg, callback) => {
	...
	for (const result of results) {
		if (result.querySelector(".search-result__result-link")) {
			...
			// 	}
			// }
			if ((url !== window.location.href + "#") && (url.indexOf("www.linkedin.com/sales/profile") > -1)) {
				let newInfos = { url }
				newInfos.name = result.querySelector(".name a").title.trim()
				newInfos.profileImageUrl = result.querySelector(".image-wrapper img").src
				newInfos.degree = result.querySelector(".details-container abbr").innerText
				newInfos.companyName = result.querySelector(".company-name").title
				newInfos.title = result.querySelector(".info-value").innerText
				newInfos.duration = result.querySelector(".info-value:nth-child(2)").innerText
				newInfos.location = result.querySelector(".info-value:nth-child(3)").innerText
...
const getSearchResults = async (tab, searchUrl, numberOfPage, query) => {
	...
	let result = []
	const selectors = ["section.search-results-container"]
	let stepCounter = 1
	...
	for (; stepCounter <= numberOfPage; i++, stepCounter++) {
		...
		// await tab.wait(1500)
		// result = result.concat(await tab.evaluate(scrapeResults))
		result = result.concat(await tab.evaluate(scrape))
