	...
	return tab.evaluate(function(arg, done) {
		...
		invites = invites.map(function accept(i) {
			if (i < arg.nbProfiles) {
				...
				if (this.querySelector("button.message-anywhere-button")) {
					// Issue #198: Send message whitout checking invitation note
					const tmp = this.querySelector("div[data-control-name=\"message\"]")
					toRet.chatMessageIndex = tmp ? tmp.id : null
				}
...
nick.newTab().then(async (tab) => {
	let { sessionCookie, numberOfProfilesToAdd, hasNoteSent, hasMutualConnections, message, csvName } = utils.validateArguments()
	...
	const selectors = [ ".js-invitation-card__invite-details-container", "section.mn-invitation-manager__no-invites", ".invitation-card.invitation-card--selectable" ]
	...
	if (invites.length > 0) {
		...
		for (const invite of invites) {
			...
			if (invite.chatMessageIndex) {
				delete invite.chatMessageIndex
			}
		}
	} else {
	...
	db.push(...utils.filterRightOuter(db, invites))
	await utils.saveResults(db, db, csvName, null, true)
})
.then(() => {
