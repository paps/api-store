...
;(async () => {
	...
	for (const one of queries) {
		...
		let search = await webSearch.search(one + " site:twitter.com")
		let link = null
		for (const res of search.results) {
			if (res.link.match(/^(?:(http|https):\/\/)?(?:www\.)?twitter\.com\/(#!\/)?\w+$/)) {
				link = res.link
				break
			}
		}
		...
		toReturn.push({ twitterUrl: link, query: one, timestamp: (new Date()).toISOString() })
		i++
	}
