...
const scrapeInstagramProfile = (arg, callback) => {
...
const getSearch = async (tab, query, engine) => {
	const [httpCode] = await tab.open(engine.baseUrl + encodeURIComponent(query + " site:instagram.com").replace(/[!'()*]/g, escape))
	if (httpCode !== 200) {
	...
	try {
		...
		return (await tab.evaluate(scrapeInstagramProfile, {selector: engine.selectors.result}))
	} catch (error) {
		...
		if (query.length > 0) {
			...
			while (loop) {
				try {
					...
					const instagramUrl = await getSearch(tab, query, engines[mode])
					result.push({instagramUrl, query})
					utils.log(`Got ${instagramUrl} for ${query}.`, "done")
					loop = false
				} catch (error) {
