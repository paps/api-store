...
// Download the csv from the spreadsheet and get all linkedIn usernames from it
const getLinkedinUsernames = data => {
	let usernames = []
	for (const url of data) {
		if (url && url.length > 1) {
			const userName = getUsername(url.trim())
			if (userName !== null) {
				usernames.push(userName)
			}
		}
	}
	// In case there is no usernames in the spreadsheet
	if (usernames.length <= 0) {
		utils.log("The spreadsheet seems to be private or empty.", "Warning")
		nick.exit(1)
	}
	return usernames
}
...
const addLinkedinFriend = async (url, tab, message, onlySecondCircle) => {
	...
	const selectors = ["button.connect.primary, button.pv-s-profile-actions--connect", "span.send-in-mail.primary, button.pv-s-profile-actions--send-in-mail", "button.accept.primary", "button.message.primary, button.pv-s-profile-actions--message.button-primary-large", "button.follow.primary", ".pv-top-card-section__invitation-pending"]
	let selector;
	try {
